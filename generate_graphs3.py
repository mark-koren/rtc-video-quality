...
def main():
  ...
  for title in sorted(lines.keys()):
    if title.split(' ')[0] == args.baseline:
      found_baseline = True
      baseline = title
  if not found_baseline:
    print("Baseline not found, setting default")
    baseline = list(sorted(lines.keys()))[0]
  rows_list = []
  for (subdir, graph_name), lines in graph_dict.items():
    bd_dict = {}
    bd_dict.update({"Name": graph_name})
    ...
    # pdb.set_trace()
    baseline_points = lines.get(baseline, None)
    R1 = []
    value1 = []
    util1 = []
    if baseline_points is not None:
      for bitrate_kbps, value, utilization in baseline_points:
        R1.append(bitrate_kbps)
        value1.append(value)
        util1.append(utilization)
    ...
    plt.close()
    if baseline_points is not None:
      rows_list.append(bd_dict)

  df = pd.DataFrame(rows_list)
  df.to_csv (r'./bjontgaard.csv', index = None, header=True)
