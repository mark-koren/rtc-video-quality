...
def prepare_clips(args, temp_dir):
  ...
  for clip in clips:
    clip['sha1sum'] = str(subprocess.check_output(['sha1sum', clip['input_file']]), sys.stdout.encoding).split(' ', 1)[0]
...
def generate_metrics(results_dict, job, temp_dir, encoded_file):
  ...
  os.close(fd)
  ssim_results = str(subprocess.check_output(['libvpx/tools/tiny_ssim', clip['yuv_file'], decoded_file, "%dx%d" % (results_dict['width'], results_dict['height']), str(temporal_skip), metrics_framestats]), sys.stdout.encoding).splitlines()
  metric_map = {
    'AvgPSNR': 'avg-psnr',
  ...
  if args.enable_vmaf:
    vmaf_results = str(subprocess.check_output(['vmaf/run_vmaf', 'yuv420p', str(results_dict['width']), str(results_dict['height']), clip['yuv_file'], decoded_file, '--out-fmt', 'json']), sys.stdout.encoding)
