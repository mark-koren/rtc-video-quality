...
def find_absolute_path(use_system_path, binary):
    ...
    if os.path.isfile(target) and os.access(target, os.X_OK):
        if use_system_path:
            print((
                  "WARNING: '%s' not in PATH (using --use-system-path), falling back on locally-compiled binary." % os.path.basename(
                      binary)))
        binary_absolute_paths[binary] = target
        return target

    sys.exit("ERROR: '%s' missing, did you run the corresponding setup script?" % (
    os.path.basename(binary) if use_system_path else target))


def entheleon_command(job, temp_dir):
    assert job['codec'] == 'entheleon'
    ...
    (fd, encoded_filename) = tempfile.mkstemp(dir=temp_dir, suffix=".ecf")
    os.close(fd)
...
def libvpx_tl_command(job, temp_dir):
    ...
    encoded_files = [{'spatial-layer': 0, 'temporal-layer': i, 'filename': "%s_%d.ivf" % (outfile_prefix, i)} for i in
                     range(job['num_temporal_layers'])]
...
def clip_arg(clip):
    ...
    return {'input_file': clip_match.group(1), 'width': int(clip_match.group(2)), 'height': int(clip_match.group(3)),
            'fps': float(clip_match.group(4)), 'file_type': 'yuv'}
    ...
    if y4m_clips:
        ...
        for clip in y4m_clips:
            ...
            with open(os.devnull, 'w') as devnull:
                subprocess.check_call(['ffmpeg', '-y', '-i', clip['input_file'], yuv_file], stdout=devnull,
                                      stderr=devnull)
            clip['yuv_file'] = yuv_file
    for clip in clips:
        clip['sha1sum'] = \
        str(subprocess.check_output(['sha1sum', clip['input_file']]), sys.stdout.encoding).split(' ', 1)[0]
...
def decode_file(job, temp_dir, encoded_file):
    ...
    with open(os.devnull, 'w') as devnull:
        if job['codec'] in ['av1', 'vp8', 'vp9']:
            ...
            subprocess.check_call([decoder, '--i420', '--codec=%s' % job['codec'], '-o', decoded_file, encoded_file,
                                   '--framestats=%s' % framestats_file], stdout=devnull, stderr=devnull)
        ...
        elif job['codec'] in ['entheleon']:
            print("Decoding Entheleon")
            clip = job['clip']
            command = [
                'entheleon/codec.py',
                '--H', str(clip['height']),
...
def generate_metrics(results_dict, job, temp_dir, encoded_file):
    ...
    os.close(fd)
    ssim_results = str(subprocess.check_output(['libvpx/tools/tiny_ssim', clip['yuv_file'], decoded_file,
    ...
    if args.enable_vmaf:
        vmaf_results = str(subprocess.check_output(
            ['vmaf/run_vmaf', 'yuv420p', str(results_dict['width']), str(results_dict['height']), clip['yuv_file'],
             decoded_file, '--out-fmt', 'json']), sys.stdout.encoding)
...
def run_command(job, command, encoded_files, job_temp_dir, encoded_file_dir):
    ...
    for i in range(len(results)):
        ...
        if encoded_file_dir:
            ...
            os.path.splitext(os.path.basename(clip['input_file']))[0], job['encoder'], job['codec'],
            job['num_spatial_layers'], job['num_temporal_layers'], job['target_bitrates_kbps'][-1],
            layer['spatial-layer'], layer['temporal-layer'], os.path.splitext(layer['filename'])[1])
...
def worker():
    ...
    while True:
        with thread_lock:
            if not jobs:
                return
            (job, command_tuple, job_temp_dir) = jobs.pop()
            command, encoded_files = command_tuple

        (results, error) = run_command(job, command, encoded_files, job_temp_dir, args.encoded_file_dir)
...
def main():
    ...
    for (encoder, codec) in args.encoders:
        ...
        elif codec in ['entheleon']:
            find_absolute_path(False, 'entheleon/codec.py')
    if args.enable_vmaf:
